...
fn test_impact_file_target_type() {
    ...
    let login_file = g
        .files
        .keys()
        .find(|f| f.contains("login.ts"))
        .expect("login.ts should exist in graph")
        .clone();
...
fn test_impact_file_impacted_files_has_auth_and_api() {
    ...
    let result = analyze_impact(&g, &login_file, 3);
    assert!(
        !result.impacted_files.is_empty(),
        "impacted files should not be empty"
    );
...
fn test_impact_depth_zero_no_transitive() {
    ...
    assert!(
        result.transitive_dependants.is_empty(),
        "depth=0 should have no transitive dependants"
    );
