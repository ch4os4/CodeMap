...
fn parse_fixture(
    ...
    let source = std::fs::read(&path).unwrap_or_else(|e| panic!("Cannot read {}: {}", path, e));
...
fn test_ts_login_exports() {
    ...
    assert!(
        names.contains(&"login"),
        "expected export 'login', got {:?}",
        names
    );
    assert!(
        names.contains(&"LoginOptions"),
        "expected export 'LoginOptions', got {:?}",
        names
    );
}
...
fn test_ts_login_classes() {
    ...
    assert!(
        classes
            .iter()
            .any(|c| c.name == "LoginOptions" && c.kind == "interface"),
...
fn test_ts_routes_imports() {
    ...
    assert!(
        imports
            .iter()
            .any(|i| i.source.contains("auth/login") && i.names.contains(&"login".to_string())),
...
fn test_ts_routes_exports() {
    ...
    assert!(
        names.contains(&"handleLogin"),
        "expected export 'handleLogin', got {:?}",
        names
    );
}
...
fn test_py_helpers_functions() {
    ...
    assert!(
        ...
        "expected 'process_data', got {:?}",
        names
    );
    assert!(
        names.contains(&"_internal_helper"),
        "expected '_internal_helper', got {:?}",
        names
    );
}
...
fn test_py_helpers_imports() {
    ...
    assert!(
        sources.contains(&"os"),
        "expected import 'os', got {:?}",
        sources
    );
    assert!(
        sources.contains(&"sys"),
        "expected import 'sys', got {:?}",
        sources
    );
    assert!(
        imports
            .iter()
            .any(|i| i.source == "os.path" && i.names.contains(&"join".to_string())),
        "expected 'from os.path import join', got {:?}",
        imports
    );
}
...
fn test_py_helpers_exports_dunder_all() {
    ...
    assert_eq!(
        names.len(),
        2,
        "__all__ should yield exactly 2 exports, got {:?}",
        names
    );
    assert!(
        names.contains(&"process_data"),
        "expected 'process_data' in __all__, got {:?}",
        names
    );
    assert!(
        names.contains(&"DataProcessor"),
        "expected 'DataProcessor' in __all__, got {:?}",
        names
    );
}
...
fn test_py_helpers_classes() {
    ...
    assert!(
        dp.methods.contains(&"__init__".to_string()),
        "expected method '__init__'"
    );
    assert!(
        dp.methods.contains(&"run".to_string()),
        "expected method 'run'"
    );
}
...
fn test_go_handler_functions() {
    ...
    let names: Vec<&str> = fns.iter().map(|f| f.name.as_str()).collect();
    assert!(
        names.contains(&"NewHandler"),
        "expected 'NewHandler', got {:?}",
        names
    );
    assert!(
        names.contains(&"ServeHTTP"),
        "expected 'ServeHTTP', got {:?}",
        names
    );
    assert!(
        names.contains(&"internalHelper"),
        "expected 'internalHelper', got {:?}",
        names
    );
}
...
fn test_go_handler_exports() {
    ...
    assert!(
        names.contains(&"NewHandler"),
        "expected exported 'NewHandler', got {:?}",
        names
    );
    assert!(
        names.contains(&"ServeHTTP"),
        "expected exported 'ServeHTTP', got {:?}",
        names
    );
    assert!(
        !names.contains(&"internalHelper"),
...
fn test_go_handler_imports() {
    ...
    assert!(
        ...
        "expected import 'fmt', got {:?}",
        sources
    );
    assert!(
        sources.contains(&"net/http"),
        "expected import 'net/http', got {:?}",
        sources
    );
    assert!(
        sources.contains(&"encoding/json"),
        "expected import 'encoding/json', got {:?}",
        sources
    );
}
...
fn test_go_handler_structs() {
    ...
    assert!(
        ...
        "expected struct 'Handler', got {:?}",
        names
    );
    assert!(
        names.contains(&"Response"),
        "expected struct 'Response', got {:?}",
        names
    );
}
...
fn test_rust_engine_functions() {
    ...
    let names: Vec<&str> = fns.iter().map(|f| f.name.as_str()).collect();
    assert!(
        names.contains(&"Engine::new"),
        "expected 'Engine::new', got {:?}",
        names
    );
    assert!(
        names.contains(&"Engine::run"),
        "expected 'Engine::run', got {:?}",
        names
    );
    assert!(
        names.contains(&"helper_function"),
        "expected 'helper_function', got {:?}",
        names
    );
    assert!(
        names.contains(&"public_function"),
        "expected 'public_function', got {:?}",
        names
    );
}
...
fn test_rust_engine_exports() {
    ...
    assert!(
        ...
        "expected pub 'Engine', got {:?}",
        names
    );
    assert!(
        names.contains(&"Status"),
        "expected pub 'Status', got {:?}",
        names
    );
    assert!(
        names.contains(&"Processable"),
        "expected pub 'Processable', got {:?}",
        names
    );
    assert!(
        names.contains(&"public_function"),
        "expected pub 'public_function', got {:?}",
        names
    );
    assert!(
        !names.contains(&"helper_function"),
        "'helper_function' should NOT be exported, got {:?}",
        names
    );
}
...
fn test_rust_engine_classes() {
    ...
    assert!(
        names.contains(&"Processable"),
        "expected trait 'Processable', got {:?}",
        names
    );
}
...
fn test_java_user_functions() {
    ...
    assert!(
        names.contains(&"User.setEmail"),
        "expected 'User.setEmail', got {:?}",
        names
    );
    assert!(
        names.contains(&"User.validate"),
        "expected 'User.validate', got {:?}",
        names
    );
}
...
fn test_java_user_imports() {
    ...
    assert!(
        ...
        "expected 'import java.util.List', got {:?}",
        imports
    );
    assert!(
        imports
            .iter()
            .any(|i| i.source == "java.util" && i.names.contains(&"Optional".to_string())),
        "expected 'import java.util.Optional', got {:?}",
        imports
    );
}
...
fn test_java_user_exports() {
    ...
    assert!(
        names.contains(&"User"),
        "expected public class 'User', got {:?}",
        names
    );
    // UserService 和 UserRole 没有 public 修饰符
    assert!(
        !names.contains(&"UserService"),
        "'UserService' has no public modifier, got {:?}",
        names
    );
}
...
fn test_java_user_classes() {
    ...
    assert!(
        names.contains(&"UserService"),
        "expected interface 'UserService', got {:?}",
        names
    );
    assert!(
        names.contains(&"UserRole"),
        "expected enum 'UserRole', got {:?}",
        names
    );
    let user = classes.iter().find(|c| c.name == "User").unwrap();
    assert!(
        user.methods.contains(&"getName".to_string()),
...
fn test_cpp_engine_functions() {
    ...
    let names: Vec<&str> = fns.iter().map(|f| f.name.as_str()).collect();
    assert!(
        names.contains(&"Engine::Engine"),
        "expected 'Engine::Engine', got {:?}",
        names
    );
    assert!(
        names.contains(&"Engine::start"),
        "expected 'Engine::start', got {:?}",
        names
    );
    assert!(
        names.contains(&"Engine::stop"),
        "expected 'Engine::stop', got {:?}",
        names
    );
    assert!(
...
fn test_cpp_engine_exports() {
    ...
    assert!(
        !names.contains(&"internalHelper"),
        "'internalHelper' is static, should NOT be exported, got {:?}",
        names
    );
    assert!(
        names.contains(&"initialize"),
        "expected export 'initialize', got {:?}",
        names
    );
}
...
fn test_cpp_engine_includes() {
    ...
    let sources: Vec<&str> = imports.iter().map(|i| i.source.as_str()).collect();
    assert!(
        sources.contains(&"iostream"),
        "expected #include <iostream>, got {:?}",
        sources
    );
    assert!(
        sources.contains(&"vector"),
        "expected #include <vector>, got {:?}",
        sources
    );
    assert!(
        sources.contains(&"utils.h"),
        "expected #include \"utils.h\", got {:?}",
        sources
    );
}
...
fn test_cpp_engine_classes() {
    ...
    assert!(
        names.contains(&"Engine"),
        "expected class 'Engine', got {:?}",
        names
    );
    assert!(
        names.contains(&"Config"),
        "expected struct 'Config', got {:?}",
        names
    );
}
...
fn test_cpp_namespace_not_in_exports() {
    ...
    assert!(
        names.contains(&"Engine"),
        "expected 'Engine' export, got {:?}",
        names
    );
    assert!(
        names.contains(&"Config"),
        "expected 'Config' export, got {:?}",
        names
    );
    assert!(
        !names.contains(&"engine"),
        "namespace 'engine' should NOT be in exports, got {:?}",
        names
    );
}
