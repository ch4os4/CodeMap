...
use codegraph::slicer::{
    generate_overview, generate_slices, get_module_slice_with_deps, save_slices,
};
use std::path::Path;
...
fn test_overview_has_project() {
    ...
    assert!(
        !overview.project.name.is_empty(),
        "overview should have project name"
    );
}
...
fn test_overview_has_scanned_at() {
    ...
    let overview = generate_overview(&g);
    assert!(
        !overview.scanned_at.is_empty(),
        "overview should have scannedAt"
    );
}
...
fn test_overview_has_summary() {
    ...
    let overview = generate_overview(&g);
    assert!(
        overview.summary.total_files > 0,
        "overview summary should have files"
    );
}
...
fn test_overview_compact_json() {
    ...
    assert!(
        json.len() < 5000,
        "overview JSON should be < 5000 chars for small fixture, got {}",
        json.len()
    );
}
...
fn test_overview_auth_module_has_file_count() {
    ...
    assert!(
        auth_mod.unwrap().file_count > 0,
        "auth module should have files"
    );
}
...
fn test_slices_has_auth_module() {
    ...
    assert!(
        slices.contains_key("auth"),
        "slices should have 'auth' module"
    );
}
...
fn test_slices_has_api_module() {
    ...
    let slices = generate_slices(&g);
    assert!(
        slices.contains_key("api"),
        "slices should have 'api' module"
    );
}
...
fn test_slices_auth_has_stats() {
    ...
    let auth_slice = &slices["auth"];
    assert!(
        auth_slice.stats.total_files > 0,
        "auth slice stats should have files"
    );
}
...
fn test_slice_with_deps_api_has_files() {
    ...
    assert!(
        !result.slice.files.is_empty(),
        "api slice should have files"
    );
}
...
fn test_save_slices_creates_overview_json() {
    ...
    assert!(
        slices_dir.join("_overview.json").exists(),
        "_overview.json should be created"
    );
...
fn test_save_slices_creates_module_files() {
    ...
    let slices_dir = output_dir.join("slices");
    assert!(
        slices_dir.join("auth.json").exists(),
        "auth.json should be created"
    );
    assert!(
        slices_dir.join("api.json").exists(),
        "api.json should be created"
    );
...
fn test_save_slices_overview_valid_json() {
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&content).expect("_overview.json should be valid JSON");
    assert!(
        parsed.get("project").is_some(),
        "overview should have 'project' field"
    );
