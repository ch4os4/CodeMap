...
fn test_query_symbol_login_function_kind() {
    ...
    let opts = QueryOptions { type_filter: None };
    let results = query_symbol(&g, "login", &opts);
    let login_fn = results
        .iter()
        .find(|r| r.kind == "function" && r.name == "login");
...
fn test_query_symbol_filter_by_function_type() {
    let g = fixture();
    let func_opts = QueryOptions {
        type_filter: Some("function".to_string()),
    };
    let results = query_symbol(&g, "login", &func_opts);
    let login_fn = results
        .iter()
        .find(|r| r.name == "login" && r.kind == "function");
    assert!(
        login_fn.is_some(),
        "should find login as function with type=function filter"
    );
}
...
fn test_query_symbol_filter_by_type_excludes_function() {
    let g = fixture();
    let type_opts = QueryOptions {
        type_filter: Some("type".to_string()),
    };
    ...
    assert!(
        login_fn.is_none(),
        "login function should not appear with type=type filter"
    );
}
...
fn test_query_dependants_auth_contains_api() {
    ...
    assert!(
        dependants.contains(&"api".to_string()),
        "auth dependants should contain 'api'"
    );
}
...
fn test_query_dependants_unknown_returns_empty() {
    ...
    assert!(
        dependants.is_empty(),
        "unknown module dependants should be empty"
    );
}
...
fn test_query_dependencies_api_contains_auth() {
    ...
    assert!(
        deps.contains(&"auth".to_string()),
        "api dependencies should contain 'auth'"
    );
}
...
fn test_query_dependencies_unknown_returns_empty() {
    ...
    assert!(
        deps.is_empty(),
        "unknown module dependencies should be empty"
    );
}
